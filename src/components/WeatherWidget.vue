<template>
  <div
    class="bg-white/80 backdrop-blur-md rounded-2xl p-4 shadow-sm border border-slate-100 flex items-center gap-4 w-fit relative group">

    <button @click="loadWeather(true)" :disabled="isLoading || isRefreshing"
    ...
    <div v-else-if="weatherData" class="flex items-center gap-3">
      ...
      <div class="flex flex-col">
        ...
        <div class="flex items-baseline gap-1">
          ...
          <span class="text-[10px] text-slate-400 font-medium">{{ isRefreshing ? 'Đang cập nhật...' : 'Hiện tại'
          }}</span>
        </div>
